...
### Crear la clase Hogar, crearle por lo menos 3 métodos y atributos, 
### considera adicionalmente el atributo número de cuartos y el de superficie en M2
...
class myHome():
    def __init__(self,rooms,surface):
        self.myrooms = rooms
        self.mysurface = surface
    def __len__(self):
        print(f'You have {self.myqty} houses with a total of {self.myrooms} rooms cover {self.mysurface} m2 of surface')
        return self.mysurface
    def __add__(self, anotherHome):
        return self.myrooms + anotherHome.myrooms

class myDepto(myHome):

    def __init__(self, rooms, surface, floor_number):
        super().__init__(rooms, surface, floor_number)
    ...
    def __add__(self, anotherHome):
        return super().__add__(anotherHome)


miHogar = myHome(4,214)
len(miHogar)

casa_de_verano = myHome(6,302)
len(casa_de_verano) 

miHogar.__add__(casa_de_verano)
miHogar.__len__()
